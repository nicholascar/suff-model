...
For example, the region with a broad boundary (also called region with indeterminate boundary, see <<BURROUGH-FRANK>>) is among the most fundamental representations of uncertainty in location. The Figure below shows a simple example of such a region: the red central core is regarded as definitely in the region; the pink penumbra is possibly in the region; and everything else is definitely not in the region.  

[id="fig-bb",width="50%",align="center"]
.A classical region with a broad boundary.
image::img/bb.svg[]

Such representations are widely used in various simple models of uncertainty, including rough sets <<WORBOYS-CEUS>> and the so-called egg-yolk model of indeterminate RCC (region connection calculus) <<COHN-GOTTS>>. The listing below provides the example data for the figure above. 
...
The idea may be extended in a variety of different ways. For example, the figure below and code listing that follow it give graphical and data views of a more complex uncertain feature created according to this model.
...
- the occurrence of endangered species, where the exact location may need to be obscured to for legal reasons in order to protect endangered native species from damage or interference (_imprecision_)
...
. <<Elements>>
.. New Semantic Web Classes and Predicates defined here and guided reuse of Classes and Predicates from other models to represent uncertainty
. <<Rules>>
